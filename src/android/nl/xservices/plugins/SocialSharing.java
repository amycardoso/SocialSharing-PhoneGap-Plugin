...
public class SocialSharing extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (ACTION_AVAILABLE_EVENT.equals(action)) {
            ...
            return doSendIntent(callbackContext, args.getString(0), args.getString(1), args.getJSONArray(2),
                    args.getString(3), "com.facebook.katana", null, false, true, "com.facebook.composer.shareintent");
            ...
            if (notEmpty(args.getString(4))) { // abid
                return shareViaWhatsAppDirectly(callbackContext, args.getString(0), args.getString(1),
                        args.getJSONArray(2), args.getString(3), args.getString(4));
            } else if (notEmpty(args.getString(5))) { // phone
                return shareViaWhatsAppDirectly(callbackContext, args.getString(0), args.getString(1),
                        args.getJSONArray(2), args.getString(3), args.getString(5));
            } else {
                return doSendIntent(callbackContext, args.getString(0), args.getString(1), args.getJSONArray(2),
            ...
            return doSendIntent(callbackContext, args.getString(0), args.getString(1), args.getJSONArray(2),
                    args.getString(3), "instagram", null, false, true);
            ...
            callbackContext.error("socialSharing." + action + " is not a supported function. Did you mean '"
                    + ACTION_SHARE_EVENT + "'?");
            return false;
        }
    }
    ...
    private boolean invokeEmailIntent(final CallbackContext callbackContext, final String message, final String subject,
        ...
        cordova.getThreadPool().execute(new SocialSharingRunnable(callbackContext) {
            public void run() {
                ...
                List<ResolveInfo> emailAppList = cordova.getActivity().getPackageManager().queryIntentActivities(draft,
                        0);
                ...
                final Intent emailAppLists = Intent
                        .createChooser(labeledIntentList.remove(labeledIntentList.size() - 1), "Choose Email App");
    ...
    private boolean shareWithOptions(CallbackContext callbackContext, JSONObject jsonObject) {
        return doSendIntent(callbackContext, jsonObject.optString("message", null),
                jsonObject.optString("subject", null),
                jsonObject.optJSONArray("files") == null ? new JSONArray() : jsonObject.optJSONArray("files"),
                jsonObject.optString("url", null), jsonObject.optString("appPackageName", null),
                jsonObject.optString("chooserTitle", null), false, false);
    }

    private boolean doSendIntent(final CallbackContext callbackContext, final String msg, final String subject,
            final JSONArray files, final String url, final String appPackageName, final String chooserTitle,
            final boolean peek, final boolean boolResult) {
    ...
    private boolean doSendIntent(final CallbackContext callbackContext, final String msg, final String subject,
            final JSONArray files, final String url, final String appPackageName, final String chooserTitle,
            final boolean peek, final boolean boolResult, final String appName) {
        ...
        cordova.getThreadPool().execute(new SocialSharingRunnable(callbackContext) {
            public void run() {
                ...
                final Intent sendIntent = new Intent(
                        hasMultipleAttachments ? Intent.ACTION_SEND_MULTIPLE : Intent.ACTION_SEND);
                ...
                final PendingIntent pendingIntent = PendingIntent.getBroadcast(
                        cordova.getActivity().getApplicationContext(), 0, receiverIntent,
                            ...
                            public void run() {
                                ...
                                for (int j = 0; j < resolveInfoList.size(); j++) {
                                    ...
                                    if (!packageName.contains("com.whatsapp") && !packageName.contains("org.telegram.messenger") && !packageName.contains("com.facebook") && !packageName.contains("com.instagram.android") 
                                        && !packageName.contains("com.google.android.gm") && !packageName.contains("com.microsoft.office.outlook") && !packageName.contains("com.appple.app.email") && !packageName.contains("com.samsung.android.email.provider")) {
    ...
    private boolean shareViaWhatsAppDirectly(final CallbackContext callbackContext, String message,
            final String subject, final JSONArray files, final String url, final String number) {
        // add the URL to the message, as there seems to be no separate field
        if (notEmpty(url)) {
            if (notEmpty(message)) {
                message += " " + url;
        ...
        cordova.getThreadPool().execute(new SocialSharingRunnable(callbackContext) {
            public void run() {
                ...
                try {
                    intent.setData(Uri.parse("https://api.whatsapp.com/send?phone=" + number + "&text="
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (_callbackContext != null) {
            ...
            case ACTIVITY_CODE_SEND__BOOLRESULT:
                _callbackContext
                        .sendPluginResult(new PluginResult(PluginResult.Status.OK, resultCode == Activity.RESULT_OK));
                break;
                ...
                try {
                    ...
                    json.put("app",
                            (ShareChooserPendingIntent.chosenComponent != null
                                    ? ShareChooserPendingIntent.chosenComponent
                                    : "")); // we need a completely different approach if we want to support this on
                                            // Android. Idea:
     ...
     * manually. Deleting them in onActivityResult is not a good idea, because for
     * example a base64 encoded file will not be available for upload to Facebook
     * (it's deleted before it's uploaded). So the best approach is deleting old
     * files when saving (sharing) a new one.
     */
